...
* build: gcc -std=c99 reduce.c -ljpeg -o reduce.o && ./reduce.o
*/
...
int norm(float* input, float* output, int len){
	float power=0;
	int ii;
	for(ii=0;ii<len;ii++){
		power += input[ii];
...
void print_img(char* rgb, int length) {
...
int main(){
	...
	/* store and export original image */ 
	unsigned char* rgb = yuyv2rgb(camera->head.start, camera->width, camera->height);
	FILE* original = fopen("original_result.jpg", "w");
	jpeg(original,rgb, camera->width, camera->height, 100);
	...
	jpeg(reduced,rgb, camera->width, camera->height, 100);
